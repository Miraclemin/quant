...
def calc_ic(df, fac_col, ret_col):
    ...
    # .notnull() 把整个表变成布尔表，非空值为True，空值为False；sum(axis=1) 则统计每行的True数量
...
def prepare_evaluate_data(factor_table, fac_freq, bench_index='000002.SH', other_name=None, samples=None, freq=None):
...
def evaluate_factor_pathways(factor_table, fac_freq, n_pathways, line, sample='全市场', bench_index='000002.SH', other_name=None, n_groups=10):
    ...
    """
    prepared = prepare_evaluate_data(factor_table, fac_freq, bench_index, other_name, samples=sample, freq=fac_freq)
    if prepared is None:
        return None
    sample_fac, sample_stk, bench_df, combinations, trade_days = prepared

    combo = combinations[0]  # 由 prepare_evaluate_data 保证此处只有单个 (sample, fac_freq)
